...
class G4GeneratorProcess(multiprocessing.Process):
    ...
    def run(self):
        ...
        while True:
            ...
            if self.tracking:
                ev.vertices, ev.photons_beg, ev.photon_parent_trackids = gen.generate_photons(ev.vertices,
...
class G4ParallelGenerator(object):
    def __init__(self, nprocesses, material, base_seed=None, tracking=False):
        ...
        self.processes = [G4GeneratorProcess(i, material, self.vertex_address, self.photon_address, seed=base_seed + i,
                                             tracking=tracking) for i in range(nprocesses)]
    ...
    def generate_events(self, vertex_iterator):
        ...
        self.processed = 0
        self.length = -1
        sender_thread = threading.Thread(target=vertex_sender,
                                         args=(vertex_iterator, self.zmq_context, self.vertex_address, self))
        sender_thread.start()
        p = zmq.Poller()
